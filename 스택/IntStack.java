...
public class IntStack {
	...
	public int push(int x) { // 원하는 값(x)을 스택에 삽입, 0번 인덱스부터 차근차근 채우기
		stk[ptr] = x; // 포인터가 가리키는 위치에 값을 삽입한 후 포인터를 1증가
		ptr++; // ptr++ : 데이터 개수 카운팅 및 그 다음 데이터는 그 다음칸에 들어갈 수 있도록 함
		return x; // 삽입된 데이터 반환
	...
	// 특정 값 몇 번 인덱스에 있는지 찾기
	public int indexOf(int x) {
		for (int i = ptr - 1; i >= 0; i--) { // 제일 위에 있는 데이터부터 0번 데이터까지 1칸씩 내려오면서 탐색
			if (stk[i] == x) { // 찾고자 하는 데이터와 같은 값을 발견했을 때
				return i; // 그 데이터가 있는 인덱스 번호 반환
			}
		}
		return -1; // 찾고자하는 값이 스택에 없는 경우에는 -1 반환
	}

	// 스택에 쌓여 있는 총 데이터 개수 반환
	public int size() {
		return ptr;
	}
	...
	public boolean isEmpty() {
		return ptr == 0;  // ptr이 0이면 (데이터가 없으면) -> true반환 / 0보다 크면 -> false 반환
